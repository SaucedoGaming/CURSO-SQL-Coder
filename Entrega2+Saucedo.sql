...
CREATE VIEW vw_estudiante_completo AS
	SELECT e.matricula,e.nombre,e.apellido,e.edad,e.correo,e.sexo,c.nombre as nombre_carrera,e.semestre,g.grado,f.siglas as siglas_facultad,f.nombre as nombre_facultad,ca.nombre as nombre_campus
	FROM estudiante e 
	JOIN carrera c ON(e.id_carrera = c.id_carrera)
	JOIN grado g ON(c.id_grado=g.id_grado)
	JOIN facultad f ON(c.id_facultad=f.id_facultad)
	JOIN campus ca ON(f.id_campus=ca.id_campus);

-- VISTA Informacion de Libro Completa
CREATE VIEW vw_libro_completo AS
	SELECT l.isbn,l.titulo,l.edicion,l.copyright,g.genero as genero,e.nombre as editorial, f.formato as formato_impresion
	FROM libro l
...
CREATE VIEW vw_status_prestamos AS
	SELECT p.id_prestamo,p.id_libro,p.matricula,p.fecha_prestamo,p.fecha_vencimiento,d.fecha_devolucion, 
		CASE
...
CREATE VIEW vw_autores_libro AS
	SELECT l.isbn,l.titulo,a.nombre as nombre_autor,a.apellido as apellido_autor FROM libro l
	LEFT JOIN autores_libro autores ON(l.isbn=autores.isbn)
	LEFT JOIN autor a ON(autores.id_autor=a.id_autor)
	ORDER BY l.isbn,ordinal_autor;

-- VISTA con cantidad de libros en existencia
CREATE VIEW vw_libros_disponibles AS
	SELECT i.isbn,l.titulo,count(i.isbn) as existencias_totales, (count(p.id_prestamo) - count(d.id_prestamo)) as prestados_actualmente,
